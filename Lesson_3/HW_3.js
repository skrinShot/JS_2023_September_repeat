...
while (i <= 20){
    document.write('<h1>Блок № ' + i + ' c довільним текстом всередині. Та індексом ' + i + '</h1>')
    i++
}
...
for (let i = 0; i < users.length; i++) {
    if (users[i].status === true) {
        document.write(`<p>${users[i].name}, ${users[i].age} років, status ${users[i].status}</p>`);
    }
}
...
//  - користувачів які старші за 30 років
document.write('<h3>Користувачі які старше за 30 років:</h3>');
for (let i = 0; i < users.length; i++) {
    if (users[i].age > 30) {
        document.write(`<p>${users[i].name}, ${users[i].age} років, status ${users[i].status}</p>`);
    }
...
for (let i = 0; i < products.length; i++){
    document.write(`
    <div class="product-card">
    <p>${products[i].title}, ${products[i].price}</p>
     <img src="${products[i].image}" alt="" class="product-image">
</div>
    
    `)
    // document.write(`<p>${products[i].title}, ${products[i].price}</p>`)
}
